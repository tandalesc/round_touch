...
static esp_err_t panel_gc9a01_draw_bitmap(esp_lcd_panel_t *panel, int x_start,
                                          int y_start, int x_end, int y_end,
                                          const void *color_data);
static esp_err_t panel_gc9a01_invert_color(esp_lcd_panel_t *panel,
                                           bool invert_color_data);
static esp_err_t panel_gc9a01_mirror(esp_lcd_panel_t *panel, bool mirror_x,
                                     bool mirror_y);
static esp_err_t panel_gc9a01_swap_xy(esp_lcd_panel_t *panel, bool swap_axes);
static esp_err_t panel_gc9a01_set_gap(esp_lcd_panel_t *panel, int x_gap,
                                      int y_gap);
...
esp_err_t
esp_lcd_new_panel_gc9a01(const esp_lcd_panel_io_handle_t io,
    ...
    ESP_GOTO_ON_FALSE(gc9a01, ESP_ERR_NO_MEM, err, TAG,
                      "no mem for gc9a01 panel");
    ...
    if (panel_dev_config->vendor_config) {
        gc9a01->init_cmds =
            ((gc9a01_vendor_config_t *)panel_dev_config->vendor_config)
                ->init_cmds;
        gc9a01->init_cmds_size =
            ((gc9a01_vendor_config_t *)panel_dev_config->vendor_config)
                ->init_cmds_size;
    }
    ...
    *ret_panel = &(gc9a01->base);
    ...
    ESP_LOGI(TAG, "LCD panel create success, version: %d.%d.%d",
             ESP_LCD_GC9A01_VER_MAJOR, ESP_LCD_GC9A01_VER_MINOR,
...
static const gc9a01_lcd_init_cmd_t vendor_specific_init_default[] = {
    ...
    {0x70,
     (uint8_t[]){0x07, 0x07, 0x04, 0x0e, 0x0f, 0x09, 0x07, 0x08, 0x03}, 9,
     0},
    {0xE8, (uint8_t[]){0x34}, 1, 0},
    {0x60,
     (uint8_t[]){0x38, 0x0b, 0x6D, 0x6D, 0x39, 0xf0, 0x6D, 0x6D}, 8, 0},
    {0x61,
     (uint8_t[]){0x38, 0xf4, 0x6D, 0x6D, 0x38, 0xf7, 0x6D, 0x6D}, 8, 0},
    {0x62,
     (uint8_t[]){0x38, 0x0D, 0x71, 0xED, 0x70, 0x70, 0x38, 0x0F, 0x71, 0xEF,
                 0x70, 0x70},
     12, 0},
    {0x63,
     (uint8_t[]){0x38, 0x11, 0x71, 0xF1, 0x70, 0x70, 0x38, 0x13, 0x71, 0xF3,
                 0x70, 0x70},
     12, 0},
    {0x64, (uint8_t[]){0x28, 0x29, 0xF1, 0x01, 0xF1, 0x00, 0x07}, 7, 0},
    {0x66,
     (uint8_t[]){0x3C, 0x00, 0xCD, 0x67, 0x45, 0x45, 0x10, 0x00, 0x00, 0x00},
     10, 0},
    {0x67,
     (uint8_t[]){0x00, 0x3C, 0x00, 0x00, 0x00, 0x01, 0x54, 0x10, 0x32, 0x98},
     10, 0},
...
static esp_err_t panel_gc9a01_init(esp_lcd_panel_t *panel) {
    ...
    ESP_RETURN_ON_ERROR(
        esp_lcd_panel_io_tx_param(io, LCD_CMD_SLPOUT, NULL, 0), TAG,
        "send command failed");
    vTaskDelay(pdMS_TO_TICKS(100));
    ESP_RETURN_ON_ERROR(
        esp_lcd_panel_io_tx_param(io, LCD_CMD_MADCTL,
                                  (uint8_t[]){gc9a01->madctl_val}, 1),
        TAG, "send command failed");
    ESP_RETURN_ON_ERROR(
        esp_lcd_panel_io_tx_param(io, LCD_CMD_COLMOD,
                                  (uint8_t[]){gc9a01->colmod_val}, 1),
        TAG, "send command failed");
    ...
    for (int i = 0; i < init_cmds_size; i++) {
        ...
        ESP_RETURN_ON_ERROR(
            esp_lcd_panel_io_tx_param(io, init_cmds[i].cmd, init_cmds[i].data,
...
static esp_err_t panel_gc9a01_draw_bitmap(esp_lcd_panel_t *panel, int x_start,
    ...
    size_t len =
        (x_end - x_start) * (y_end - y_start) * gc9a01->fb_bits_per_pixel / 8;
    ESP_RETURN_ON_ERROR(
...
static esp_err_t panel_gc9a01_disp_on_off(esp_lcd_panel_t *panel,
                                          bool on_off) {
    gc9a01_panel_t *gc9a01 = __containerof(panel, gc9a01_panel_t, base);
    esp_lcd_panel_io_handle_t io = gc9a01->io;
